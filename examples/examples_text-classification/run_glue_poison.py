...
def main():
    ...
    if data_args.task_name is not None:
        ...
        if is_regression:
            ...
            num_labels = len(label_list)
    data_files = {"train": model_args.poisoned_train_file, "validation":model_args.clean_dev_file}
    ...
    poisoned_dict = {}
    poisoned_dict['idx']=[]
    poisoned_dict['sentence']=[]
    poisoned_dict['label']=[]
    for i in range(0,len(examples)):
        poisoned_dict['idx'].append(i)
        poisoned_dict['sentence'].append(examples[i][0])
        poisoned_dict['label'].append(examples[i][1])
    train_dataset = Dataset.from_dict(poisoned_dict)
    ...
    for i in range(0,len(examples)):
        ...
        poisoned_dict['sentence'].append(examples[i][0])
        poisoned_dict['label'].append(examples[i][1])
    val_dataset = Dataset.from_dict(poisoned_dict)
    raw_datasets['train'] = train_dataset
    raw_datasets['validation'] = val_dataset
    print("original:",raw_datasets['train'][0])
    print("raw_datasets['train']:",raw_datasets['train'])
    print("raw_datasets['validation']:",raw_datasets['validation'])
    ...
    examples = []
    path = model_args.poisoned_test_file
    with open(path, 'r') as f:
        reader = csv.DictReader(f, delimiter='\t')
        for idx, example_json in enumerate(reader):
            text_a = example_json['sentence'].strip()
    ...
    poison_test_dataloader = DataLoader(dataset = poison_test_data,batch_size=128,shuffle=False,collate_fn=collate_fn)
    model = trainer.model
    model.eval()
    preds, labels = [], []
    ...
    with torch.no_grad():
        for idx, batch in enumerate(clean_test_dataloader):
            ...
            batch_inputs = tokenizer(text, padding=True, truncation=True, max_length=max_seq_length,return_tensors="pt")
    ...
    score_clean = classification_metrics(preds, labels,metric="accuracy")
    preds,labels = [],[]
    with torch.no_grad():
    ...
    print("the clean testing acc is:",score_clean)
    print("the testing asr is:",score_asr)
