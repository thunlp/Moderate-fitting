...
class DeltaBase(nn.Module, SaveLoadMixin):
    ...
    def add_all_delta_to_backbone(self, 
        ...
        if reparams is not None:
            if hasattr(self, "delta_params") and self.delta_params:
                reparams.instantiate(module_list=self.delta_modules, param_list=self.delta_params)
                self.delta_modules = None
                self.delta_params = None
            else:
                reparams.instantiate(module_list=self.delta_modules)
                self.delta_modules = None
            self.reparams = reparams
            self.insert_sequential_module(
                backbone.get_encoder() if backbone.config.is_encoder_decoder else backbone,
                delta_module=reparams,
                delta_name="reparams",
            )
